...
<div id="section-body" class="container" style="padding-right: 20px;">
	<div class="row">
		<form class="form-horizontal" role="form" action="<?php echo framework::link_to("admin/{$action}_paper_submit"); if (isset($id)) echo '&id='.$id; if (!empty($current_lang)) echo '&lang='.$current_lang;?>" method="post" enctype="multipart/form-data">
			<div class="col-lg-12">
								...
								<?php for($i=1;$i<=1;$i++) { ?>
									<td valign="bottom">  
										...
										<?php if (isset($$imagename)) if ($$imagename !='') { ?>
											<img class="admin-product-image" alt="Image not available" src="<?php echo framework::resolve("static/images/papers/".$$imagename); ?>"><br/>
				...
				<div class="form-group text-center">
					...
					<a href="<?php echo framework::link_to('admin/papers'); ?>" class="btn btn-default btn-lg">Cancel</a>
				</div>
			</div>
		</form>
	</div>
