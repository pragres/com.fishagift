...
class Email {
	 ...
	 * @param string $to
	 */
	static function sendNewPassword($to, $password){
		$lang = framework::session_get('language');
		
		include_once framework::resolve("packages/email/i18n/$lang/reset_password.php");
		
		$subject = $i18n['subject'];
		
		ob_start();
		include framework::resolve("packages/email/view/reset_password.tpl");
		$message = ob_get_contents();
		ob_clean();
